...
для создания групп пользователей "Модератор продуктов" и "Контент-менеджер" необходимо выполнить следующие команды: 
```bash
# Создать модераторов продуктов
python manage.py create_moderators
```

```bash
# Создать контент-менеджеров
